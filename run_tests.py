...
def find_tests(theme):
    ...
    for dirpath, dirnames, filenames in os.walk("./assets/{}/" .format(theme.dir)):
...
def main():
    ...
    for test in sorted(tests):
        ...
        subprocess.check_output(['./build/FOSSSim/FOSSSim', '-s', '{}' .format(test),
                                 '-d', '0', '-o', './test_output.bin'])

        oracle = subprocess.check_output(['./oracle/FOSSSimOracle{}' .format(theme.upper()),
                                          '-s', '{}' .format(test), '-d', '0', '-i', './test_output.bin'],
