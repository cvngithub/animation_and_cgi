...
{	
    ...
    qefinal = qe;
    qdotefinal = qdote;
    for(int i = 0; i < m_maxiters && !collisions.empty(); ++i) {
        VectorXs newqe = qefinal;
        VectorXs newqdote = qdotefinal;
        applyImpulses(scene, collisions, qs, qefinal, qdotefinal, dt, newqe, newqdote);
        qefinal = newqe;
        qdotefinal = newqdote;
...
void HybridCollisionHandler::performFailsafe(const TwoDScene &scene, const VectorXs &qs, const ImpactZone &zone, double dt, VectorXs &qe, VectorXs &qdote)
{
    ...
    VectorXs dx = qe - qs;
    Vector2s xcm = Vector2s::Zero(); 
    Vector2s dxcm = Vector2s::Zero();
    ...
    for (std::set<int>::iterator it = zone.m_verts.begin(); it != zone.m_verts.end(); ++it) {
        ...
        xcm += qs.segment<2>(2*(*it)) * M[2*(*it)];
        dxcm += dx.segment<2>(2*(*it)) * M[2*(*it)];
        totalmass += M[2*(*it)];
    }
    ...
    if (hasFixed || zone.m_halfplane) {
        for (std::set<int>::iterator it = zone.m_verts.begin(); it != zone.m_verts.end(); ++it) {
            ...
            qdote.segment<2>(2*(*it)) = Vector2s::Zero();
        }
    } else {
        xcm /= totalmass;
        dxcm /= totalmass;
        ...
        double I = 0.0;
        for (std::set<int>::iterator it = zone.m_verts.begin(); it != zone.m_verts.end(); ++it) {
            Vector2s xs_xcm = qs.segment<2>(2*(*it)) - xcm; // xs - xcm
            Vector2s dx_dxcm = dx.segment<2>(2*(*it)) - dxcm; // dx - dxcm
            L += M[2*(*it)] * (xs_xcm[0] * dx_dxcm[1] - xs_xcm[1] * dx_dxcm[0]);
            I += M[2*(*it)] * pow(xs_xcm.norm(), 2);
        }
        double wcm = L / I; // angular velocity

        for (std::set<int>::iterator it = zone.m_verts.begin(); it != zone.m_verts.end(); ++it) {
            Vector2s xs_xcm = qs.segment<2>(2*(*it)) - xcm;
            Vector2s _xs_xcm = xs_xcm;
            std::swap(_xs_xcm[0], _xs_xcm[1]);
            _xs_xcm[0] *= -1;
            qe.segment<2>(2*(*it)) = xcm + dxcm + cos(wcm)*(xs_xcm) + sin(wcm)*(_xs_xcm);
