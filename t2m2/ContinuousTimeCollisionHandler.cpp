...
bool ContinuousTimeCollisionHandler::detectParticleParticle(const TwoDScene &scene, const VectorXs &qs, const VectorXs &qe, int idx1, int idx2, Vector2s &n, double &time)
{
    ...
    position_polynomial.push_back( -2*(dx2-dx1).dot(x2-x1) );
    position_polynomial.push_back( (r1+r2)*(r1+r2) - (x2-x1).dot(x2-x1) );

    velocity_polynomial.push_back( -(dx1-dx2).dot(dx1-dx2) );
...
bool ContinuousTimeCollisionHandler::detectParticleEdge(const TwoDScene &scene, const VectorXs &qs, const VectorXs &qe, int vidx, int eidx, Vector2s &n, double &time)
{
    ...
    double rr = pow(r1 + r2, 2);

    position_polynomial.push_back( pow(c, 2) - i * f );
    position_polynomial.push_back( 2 * c * b - 2 * e * i - 2 * h * f );
    position_polynomial.push_back( 2 * c * a + pow(b, 2) - 4 * e * h - f * g - i * d + rr * i );
    position_polynomial.push_back( 2 * a * b - 2 * h * d - 2 * e * g + 2 * rr * h );
    position_polynomial.push_back( pow(a, 2) - d * g + rr * g );

    alpha_greater_than_zero_polynomial.push_back(c);
    alpha_greater_than_zero_polynomial.push_back(b);
    alpha_greater_than_zero_polynomial.push_back(a);

    alpha_less_than_one_polynomial.push_back((dx1 - dx3).dot(dx2 - dx3));
    alpha_less_than_one_polynomial.push_back((x1 - x3).dot(dx2 - dx3) + (dx1 - dx3).dot(x2 - x3));
    ...
    // Your implementation here should compute n, and examine time to decide the return value
    if (time <= 1) {       
        double alpha = (x1 - x2 + time * (dx1 - dx2)).dot(x3 - x2 + time * (dx3 - dx2)) / (x3 - x2 + time * (dx3 - dx2)).dot(x3 - x2 + time * (dx3 - dx2));
        n = (x2 + time * dx2) + alpha * (x3 - x2 + time * (dx3 - dx2)) - (x1 + time * dx1);
        return true;
    }
...
bool ContinuousTimeCollisionHandler::detectParticleHalfplane(const TwoDScene &scene, const VectorXs &qs, const VectorXs &qe, int vidx, int pidx, Vector2s &n, double &time)
{
    ...
    // Your implementation here should fill the polynomials with right coefficients
    position_polynomial.push_back(-dx1.dot(np) * dx1.dot(np));
    position_polynomial.push_back(2 * (xp - x1).dot(np) * dx1.dot(np));
    position_polynomial.push_back(-(xp - x1).dot(np) * (xp - x1).dot(np) + r * r * np.dot(np));
